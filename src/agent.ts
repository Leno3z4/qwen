...
const tools = [
  ...
  { type: 'function' as const, function: { name: 'get_state', description: 'Get a fresh authenticated Perpl portfolio state including ALL current accounts, open orders, and ALL open positions. Use before every trade decision.', parameters: { type: 'object', properties: {}, additionalProperties: false } } },
  { type: 'function' as const, function: { name: 'get_market_candles', description: 'Get native Perpl OHLCV candles for one market and timeframe.', parameters: { type: 'object', properties: { market_id: { type: 'integer', minimum: 1 }, resolution_seconds: { type: 'integer', minimum: 60 }, from_ms: { type: 'integer', minimum: 0 }, to_ms: { type: 'integer', minimum: 0 } }, required: ['market_id', 'resolution_seconds', 'from_ms', 'to_ms'], additionalProperties: false } } },
  ...
  { type: 'function' as const, function: { name: 'get_trading_memory', description: 'Read recent agent trading memory and outcomes.', parameters: { type: 'object', properties: { limit: { type: 'integer', minimum: 1, maximum: 6 } }, additionalProperties: false } } },
  { type: 'function' as const, function: { name: 'web_research', description: 'Research external market context when useful. Do not use this instead of Perpl-native state/market data.', parameters: { type: 'object', properties: { query: { type: 'string' }, max_results: { type: 'integer', minimum: 1, maximum: 3 } }, required: ['query'], additionalProperties: false } } },
  { type: 'function' as const, function: { name: 'place_order', description: 'Place a live Perpl order. Use only after checking current state and sizing the trade within configured risk limits.', parameters: { type: 'object', properties: { mkt: { type: 'integer', minimum: 1 }, t: { type: 'integer', enum: [1, 2] }, s: { type: 'number', minimum: 0 }, lv: { type: 'number', minimum: 0.01 }, fl: { type: 'integer', minimum: 0 }, p: { type: 'number', minimum: 0 }, a: { type: 'number' }, ms: { type: 'number' }, tif: { type: 'integer' }, tp: { type: 'number' }, tpc: { type: 'number' }, tr: { type: 'number' }, lp: { type: 'integer', minimum: 0 }, bf: { type: 'integer' } }, required: ['mkt', 't', 's', 'lv'], additionalProperties: false } } },
  { type: 'function' as const, function: { name: 'manage_position', description: 'Reduce or close one existing Perpl position by exact position id (pid). Use this for precise per-position management. Repeat for multiple positions in the same cycle when justified.', parameters: { type: 'object', properties: { position_id: { type: 'integer', minimum: 1 }, action: { type: 'string', enum: ['reduce', 'close'] }, size: { type: 'number', minimum: 0 } }, required: ['position_id', 'action'], additionalProperties: false } } },
...
async function runTool(name: string, args: Json): Promise<unknown> {
  ...
  if (name === 'get_markets') result = await perpl.getMarkets();
    ...
    if (!Number.isFinite(leverageHundredths) || leverageHundredths <= 0) throw new Error('Order leverage must be positive');
    const leverage = leverageHundredths;
    if (leverage > maxLeverage) throw new Error(`Order leverage ${leverage}x exceeds MAX_LEVERAGE=${maxLeverage}x`);
    ...
    if (![1, 2].includes(orderType)) throw new Error(`Unsupported entry order type ${orderType}`);
    const currentState: any = await perpl.getState();
    const account = currentState?.account ?? currentState?.accounts?.[0];
    if (!account) throw new Error('No active Perpl account found');
    const balanceRaw = Number(account?.b ?? account?.balance ?? 0);
    const availableRaw = Number(account?.ab ?? account?.available_balance ?? balanceRaw);
    if (!Number.isFinite(balanceRaw) || balanceRaw <= 0) throw new Error('Perpl account balance is unavailable or non-positive');
    if (!Number.isFinite(availableRaw) || availableRaw <= 0) throw new Error('Perpl available balance is unavailable or non-positive');
    result = await perpl.placeOrder({ mkt: Number(args.mkt), t: orderType, s: size, lv: leverage, fl: Number(args.fl ?? 0), p: Number(args.p ?? 0), a: args.a === undefined ? undefined : Number(args.a), ms: args.ms === undefined ? undefined : Number(args.ms), tif: args.tif === undefined ? undefined : Number(args.tif), tp: args.tp === undefined ? undefined : Number(args.tp), tpc: args.tpc === undefined ? undefined : Number(args.tpc), tr: args.tr === undefined ? undefined : Number(args.tr), lp: args.lp === undefined ? undefined : Number(args.lp), bf: args.bf === undefined ? undefined : Number(args.bf) } as any);
  } else if (name === 'manage_position') {
    if (!tradingEnabled) throw new Error('Trading is disabled by TRADING_ENABLED=false');
    const positionId = Number(args.position_id);
    if (!Number.isInteger(positionId) || positionId <= 0) throw new Error('position_id must be a positive integer');
